...
## CSS 为什么比 js 在浏览器上更加高效

### 8.1 浏览器的渲染流程

1. 在 DOM 树生成之后, 我们还要经历布局, 分层, 绘制, 合成, 显示等阶段, 如果我们用 js 操作 DOM 那么就会经历**重排** , **重绘**, 和**合成** 三个阶段. 通常渲染路径越长, 生成图像所花费的时间就越多

2. chrome 在合成之前做了很多 **分层**的操作, 来优化动画, 渐变, 平移或者缩放, 而这些都是用显卡来完成的. 所以 css 这些功能要比 js 更快

### 8.2 chrome 是如何实现分层和合成机制的

1. 浏览器在**绘制阶段**其实是将绘制指令组合成一个列表, 比如一个图层要设置的背景为黑色, 并且还要在中间画一个圆形, 那么绘制过程会生成 `paint backGroundColor:black | paint circle` 这样的绘制指令, 绘制过程就完成了
2. 合成操作是在合成线程完成的, 这就是意味着在执行图片合成的操作时候, 不会影响主线程, 这就是为什么 js 卡住了, css 动画依旧能执行的原因, 而且 css 效率比 js 更快.

### 8.3 chrome 上的分块机制

 1. 合成线程会将每个图层分割为大小固定的图块, 然后优先绘制靠近视口的图块, 这样就可以大大加速页面的显示速度.
 2. 纹理上传: 就是先何晨给一个低分辨率,然后合成器继续绘制正常比例的内容, 当正常比例的内容绘制完成之后, 在替换掉低分辨率内容

### 8.4 css 动画比 javascript 更高效

1. 我们可以用 `will-change`, 来提前告诉渲染引擎, 让他为该元素准备独立的层, 在利用分层和合成技术, 我们渲染引擎在 css 上用是`合成线程`去帮助我们完成 css 动画的转变, 不占用主线程的资源, 这样就大大提升了渲染效率
2. `will-change`来提前告诉渲染引擎, 然他准备单独的层专门负责, 但是占用的内存也会大大增加, 所以恰当使用
